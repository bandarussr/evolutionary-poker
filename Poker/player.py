...
class Player:
    ...
    def make_decision(self, bet_size, min_raise, community_cards):
        ...
        elif self.chips.total_value() < bet_size.total_value():
    ...
    def place_bet(self, bet):
        ...
        if self.chips.total_value() >= bet.total_value():
            # Transfer chips directly from player to their bet
            self.bet.transfer_chips(self.chips, bet)
        else:
            # Player doesn't have enough - go all in
            all_in_bet = self.chips.copy()  # Copy all chips they have
            self.bet.transfer_chips(self.chips, all_in_bet)
